...
page = st.radio("Navigation", ["🏠 Home", "📖 Journal"], horizontal=True)
...
if page == "🏠 Home":
    ...
    with col1:
        if st.session_state.show_info:
            with st.expander("ℹ About Bot", expanded=True):
                st.markdown("""
                ...
                """)
    with col2:
        st.button("ℹ About Bot", on_click=lambda: st.session_state.update(show_info=not st.session_state.show_info))
